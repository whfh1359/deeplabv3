...
def run_model(gpu=1,
    ...
    ckpt_path = '/app2/0629_new_img_half_augment_add_img_rate_data_low_learning_rate/iou/1_0.81617_unet'
    pretrain_use = True
    ...
    def my_load(img_path,mask_path,aug):
        ...
        for one_img, one_mask in zip(img_path, mask_path):
            ...
            save_name = os.path.basename(one_img)
            ...
            # temp_mask *= 255
            # cv2.imwrite('/app3/separate_img_result/mask' + save_name, temp_mask)
            ...
            # rightmask *= 255
            #
            # temp_leftmask = np.copy(leftmask)
            # temp_rightmask = np.copy(rightmask)
            # temp_leftmask = cv2.cvtColor(temp_leftmask, cv2.COLOR_GRAY2BGR)
            # temp_rightmask = cv2.cvtColor(temp_rightmask, cv2.COLOR_GRAY2BGR)
            #
            # left_superimposed_img = cv2.addWeighted(leftimg, 0.8, temp_leftmask, 0.4, 0)
            # right_superimposed_img = cv2.addWeighted(rightimg, 0.8, temp_rightmask, 0.4, 0)
            # cv2.imwrite('/app3/separate_img_result/L_' + save_name, left_superimposed_img)
            # # cv2.imwrite('/app3/separate_img_result/L_real_mask' + save_name, leftmask)
            # cv2.imwrite('/app3/separate_img_result/R_' + save_name, right_superimposed_img)
            # cv2.imwrite('/app3/separate_img_result/R_real_mask' + save_name, rightmask)
            if dump:
                dump_mask = (255-mask)
            ...
            # plt.show()
                ...
                # mask = np.expand_dims(mask, axis=2)
                leftmask = np.expand_dims(leftmask, axis=2)
                rightmask = np.expand_dims(rightmask, axis=2)
            ...
            if aug:
                image_aug = random.randint(0, 6)
                if image_aug == 0:
                    # print(image_aug)
                    pass
                elif image_aug == 1:
                    # print(image_aug)
                    leftimg = clahe(leftimg)
                    rightimg = clahe(rightimg)
                elif image_aug == 2:
                    # print(image_aug)
                    leftimg = normalize(leftimg)
                    rightimg = normalize(rightimg)

                elif image_aug == 3:
                    # print(image_aug)
                    leftimg = clahe(leftimg)
                    leftimg = sharpen(leftimg)
                    rightimg = clahe(rightimg)
                    rightimg = sharpen(rightimg)
                elif image_aug == 4:
                    leftimg = gaussian_noise(leftimg)
                    rightimg = gaussian_noise(rightimg)
                elif image_aug == 5:
                    random_gamma = random.uniform(0.5, 1.5)
                    leftimg = gamma_correction(leftimg, random_gamma)
                    rightimg = gamma_correction(rightimg, random_gamma)

                rflip = random.randint(0, 1)
                if rflip:
                    leftimg = np.flip(leftimg, 1)
                    rightimg = np.flip(rightimg, 1)
                    leftmask = np.flip(leftmask, 1)
                    rightmask = np.flip(rightmask, 1)

                flag_rotation = random.randint(0, 1)
                if flag_rotation:
                    r_rotation = random.randint(0, 20)
                    left_con_img = np.concatenate((leftimg, leftmask), axis=2)
                    right_con_img = np.concatenate((rightimg, rightmask), axis=2)
                    left_h_size = left_con_img.shape[0]
                    left_w_size = left_con_img.shape[1]
                    left_h_rsize = random.randint(np.floor(0.9 * left_h_size), left_h_size)
                    left_w_rsize = random.randint(np.floor(0.9 * left_w_size), left_w_size)
                    right_h_size = right_con_img.shape[0]
                    right_w_size = right_con_img.shape[1]
                    right_h_rsize = random.randint(np.floor(0.9 * right_h_size), right_h_size)
                    right_w_rsize = random.randint(np.floor(0.9 * right_w_size), right_w_size)

                    left_w_s = random.randint(0, left_w_size - left_w_rsize)
                    left_h_s = random.randint(0, left_h_size - left_h_rsize)
                    right_w_s = random.randint(0, right_w_size - right_w_rsize)
                    right_h_s = random.randint(0, right_h_size - right_h_rsize)

                    left_con_img = imutils.rotate(left_con_img, r_rotation)
                    right_con_img = imutils.rotate(right_con_img, r_rotation)

                    leftimg = left_con_img[left_h_s:left_h_s + left_h_size, left_w_s:left_w_s + left_w_size, :3]
                    leftmask = left_con_img[left_h_s:left_h_s + left_h_size, left_w_s:left_w_s + left_w_size, 3:]

                    rightimg = right_con_img[right_h_s:right_h_s + right_h_size, right_w_s:right_w_s + right_w_size, :3]
                    rightmask = right_con_img[right_h_s:right_h_s + right_h_size, right_w_s:right_w_s + right_w_size, 3:]


            leftimg = cv2.resize(leftimg, (input_size, input_size))
            leftmask = cv2.resize(leftmask, (input_size, input_size))
            leftmask = np.expand_dims(leftmask, axis=2)
            rightimg = cv2.resize(rightimg, (input_size, input_size))
            rightmask = cv2.resize(rightmask, (input_size, input_size))
            rightmask = np.expand_dims(rightmask, axis=2)

            # print(img.shape)
            # print(mask.shape)

            img_data.append(np.array(leftimg))
            img_data.append(np.array(rightimg))
            mask_data.append(np.array(leftmask))
            mask_data.append(np.array(rightmask))

        return np.array(img_data), np.array(mask_data)
    def my_load_XY(set_path,aug):
        img_data = []
        mask_data = []

        for one_img, one_mask in set_path:
            # print('one_img',one_img)
            # print('one_mask',one_mask)
            save_name = os.path.basename(one_img)
            img = cv2.imread(one_img)
    ...
    for epoch in range(epoch_cnt):
        ...
        for i in range(train_batch_cnt):
            ...
            batch_XY = next_batch(train_set, batch_size, i)
            # print(batch_X)
            # print(batch_Y)
            # print(batch_cY)

            # batch_X, batch_Y = my_load(batch_X,batch_Y, True)
            batch_X, batch_Y = my_load_XY(batch_XY, True)
