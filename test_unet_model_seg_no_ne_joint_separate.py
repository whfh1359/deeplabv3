...
def run_model(gpu=1,
    ...
    for i in range(val_batch_cnt):
        ...
        if 'test' in img_path:
            ...
            # left_superimposed_img = cv2.addWeighted(left_x_original, 0.8, left_heatmap, 0.2, 0)
            # right_superimposed_img = cv2.addWeighted(right_x_original, 0.8, right_heatmap, 0.2, 0)
            gap = int((x_original.shape[1] * 0.5) - (x_original.shape[1] * 0.4))
            left_superimposed_img = left_heatmap[:, 0:left_y_predict_resize.shape[1] - gap]
            right_superimposed_img = left_heatmap[:, 0 + gap:right_y_predict_resize.shape[1]]
