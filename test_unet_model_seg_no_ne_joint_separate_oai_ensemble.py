...
def run_model(gpu=1,
    ...
    def my_load(img_path, img_precess):
        left_img_data = []
        left_mask_data = []
        right_img_data = []
        right_mask_data = []
        ...
        for ori_img in img_path:
            # print(ori_img)
            # save_name = os.path.basename(ori_img)
            ...
            # left_img = img[:, :int(img.shape[1] * 0.5)]
            # # leftmask = mask[:, :int(mask.shape[1] * 0.5)]
            # right_img = img[:, int(img.shape[1] * 0.5):img.shape[1]]
            x_minus = (img.shape[1] - img.shape[0]) / 2
            x_minus = int(x_minus)
            x_original_1 = img.copy()
            if img.shape[1] == img.shape[0] + (x_minus*2):
                constant = cv2.copyMakeBorder(img, x_minus, x_minus, 0, 0, cv2.BORDER_CONSTANT)
            else:
                constant = cv2.copyMakeBorder(img, x_minus, x_minus+1, 0, 0, cv2.BORDER_CONSTANT)
            print(constant.shape)

            # cv2.imwrite('/app3/test_padding/' + os.path.basename(ori_img), constant)
            left_img = constant[:, 0:int(constant.shape[1] * 0.6)]

            right_img = constant[:, int(constant.shape[1] * 0.4):constant.shape[1]]

            # rightmask = mask[:, int(mask.shape[1] * 0.5):mask.shape[1]]
            ...
            # print(img.shape)
            # print(mask.shape)
            # cv2.imwrite('/app3/separate_img_result/L' + save_name, left_img)
            # cv2.imwrite('/app3/separate_img_result/R' + save_name, right_img)
            clahe_left_img = clahe(left_img)
            nor_left_img = normalize(left_img)
            clahe_right_img = clahe(right_img)
            nor_right_img = normalize(right_img)

            left_img = cv2.resize(left_img, (input_size, input_size))
            clahe_left_img = cv2.resize(clahe_left_img, (input_size, input_size))
            nor_left_img = cv2.resize(nor_left_img, (input_size, input_size))
            # left_mask = cv2.resize(left_mask, (input_size, input_size))
            # left_mask = np.expand_dims(left_mask, axis=2)
            right_img = cv2.resize(right_img, (input_size, input_size))
            clahe_right_img = cv2.resize(clahe_right_img, (input_size, input_size))
            nor_right_img = cv2.resize(nor_right_img, (input_size, input_size))
            ...
            temp_right_list = []
            # np.array()
            # left_img = np.squeeze(left_img, 0)
            # nor_left_img = np.squeeze(nor_left_img, 0)
            # clahe_left_img = np.squeeze(clahe_left_img, 0)
            temp_left_list.append(left_img)
            temp_left_list.append(nor_left_img)
            temp_left_list.append(clahe_left_img)

            # right_img = np.squeeze(right_img, 0)
            # nor_right_img = np.squeeze(nor_right_img, 0)
            # clahe_right_img = np.squeeze(clahe_right_img, 0)
            temp_right_list.append(right_img)
            temp_right_list.append(nor_right_img)
            temp_right_list.append(clahe_right_img)
            # left_img_data = np.hstack([left_img, clahe_left_img, nor_left_img])
            # right_img_data = np.hstack([right_img, clahe_right_img, nor_right_img])
            ...
            #     right_mask_data)
            # return np.array(left_img_data), np.array(right_img_data)
            # print(left_img_data.shape)
            # print(right_img_data.shape)
            return np.array(temp_left_list), np.array(temp_right_list)
    ...
    if do_ne:
        ...
        po_X_val = list_load(val_img_path)
    ...
    def val_next_batch(batch_s, iters):
        ...
        return X_val[count:(count + batch_s)]
    ...
    # model.load_weights('/app2/0504_add_data_original_learning_separate/iou/1_0.79411_unet')
    ...
    for i in range(val_batch_cnt):
        ...
        val_batch_X = val_next_batch(batch_size, i)
        # val_batch_Y = val_next_batch(Y_val, batch_size)
        print(val_batch_X)
        # print(val_batch_Y)

        leftimg_X, rightimg_X = my_load(val_batch_X, 0)
        # flip_leftimg_X, flip_rightimg_X = my_load(val_batch_X, 1)
        # print(leftimg_X)
        ...
        # flip_leftimg_X = np.squeeze(flip_leftimg_X, 0)
        # flip_leftimg_Y = np.squeeze(flip_leftimg_Y, 0)
        # flip_rightimg_X = np.squeeze(flip_rightimg_X, 0)
        # flip_rightimg_Y = np.squeeze(flip_rightimg_Y, 0)
        # flip_left_val_batch_X.append(flip_leftimg_X)
        # flip_left_val_batch_Y.append(flip_leftimg_Y)
        # flip_right_val_batch_X.append(flip_rightimg_X)
        # flip_right_val_batch_Y.append(flip_rightimg_Y)
        #
        # left_val_batch_X = np.array(left_val_batch_X)
        # # left_val_batch_Y = np.array(left_val_batch_Y)
        # right_val_batch_X = np.array(right_val_batch_X)
        # # right_val_batch_Y = np.array(right_val_batch_Y)
        #
        # # flip_left_val_batch_X = np.array(flip_left_val_batch_X)
        # # flip_left_val_batch_Y = np.array(flip_left_val_batch_Y)
        # # flip_right_val_batch_X = np.array(flip_right_val_batch_X)
        # # flip_right_val_batch_Y = np.array(flip_right_val_batch_Y)
        # # val_batch_X = np.squeeze(val_batch_X,0)
        # # val_batch_Y = np.squeeze(val_batch_Y,0)
        # # print(flip_left_val_batch_X.shape)
        # # print(flip_right_val_batch_X.shape)
        left_val_predict = model(leftimg_X)
        right_val_predict = model(rightimg_X)
        # flip_left_val_predict = model(flip_left_val_batch_X)
        # flip_right_val_predict = model(flip_right_val_batch_X)

        img_path = val_batch_X[0]
        x_original = cv2.imread(img_path)
        x_minus = (x_original.shape[1] - x_original.shape[0]) / 2
        x_minus = int(x_minus)
        x_original_1 = x_original.copy()
        if x_original.shape[1] == x_original.shape[0] + (x_minus * 2):
            x_original = cv2.copyMakeBorder(x_original, x_minus, x_minus, 0, 0, cv2.BORDER_CONSTANT)
        else:
            x_original = cv2.copyMakeBorder(x_original, x_minus, x_minus + 1, 0, 0, cv2.BORDER_CONSTANT)
        ...
        left_x_original = x_original[:, 0:int(x_original.shape[1] * 0.6)]

        right_x_original = x_original[:, int(x_original.shape[1] * 0.4):x_original.shape[1]]
        # leftimg = img[:, 0:int(img.shape[1] * 0.6)]
        # leftmask = left_mask[:, 0:int(left_mask.shape[1] * 0.6)]
        # rightimg = img[:, int(img.shape[1] * 0.4):img.shape[1]]
        # rightmask = right_mask[:, int(right_mask.shape[1] * 0.4):right_mask.shape[1]]
        ...
        # flip_left_val_predict = np.flip(flip_left_val_predict, 2)
        # flip_right_val_predict = np.flip(flip_right_val_predict, 2)
        left_val_predict = np.squeeze(left_val_predict, axis=3)
        right_val_predict = np.squeeze(right_val_predict, axis=3)
        # flip_left_val_predict = np.squeeze(flip_left_val_predict, axis=3)
        # flip_right_val_predict = np.squeeze(flip_right_val_predict, axis=3)

        temp_left_predict = np.copy(left_val_predict)
        temp_right_predict = np.copy(right_val_predict)
        # temp_left_predict = np.squeeze(temp_left_predict)
        # temp_right_predict = np.squeeze(temp_right_predict)

        # flip_temp_left_predict = np.copy(flip_left_val_predict)
        # flip_temp_right_predict = np.copy(flip_right_val_predict)
        # flip_temp_left_predict = np.squeeze(flip_temp_left_predict)
        # flip_temp_right_predict = np.squeeze(flip_temp_right_predict)
        #
        # temp_left_batch_Y = np.copy(left_val_batch_Y)
        # temp_right_batch_Y = np.copy(right_val_batch_Y)
        # temp_left_batch_Y = np.squeeze(temp_left_batch_Y, axis=3)
        # temp_right_batch_Y = np.squeeze(temp_right_batch_Y, axis=3)
        # temp_left_batch_Y = np.squeeze(temp_left_batch_Y)
        # temp_right_batch_Y = np.squeeze(temp_right_batch_Y)

        # temp_left_predict = flip_temp_left_predict
        # temp_right_predict = flip_temp_right_predict
        # flip_temp_left_predict = (temp_left_predict + flip_temp_left_predict) / 2
        # flip_temp_right_predict = (temp_right_predict + flip_temp_right_predict) / 2
        # temp_left_predict = np.copy(left_val_predict)
        # temp_right_predict = np.copy(right_val_predict)

        left_val_predict = left_val_predict
        right_val_predict = right_val_predict
        # flip_temp_left_predict[flip_temp_left_predict >= 0.5] = 1.
        # flip_temp_right_predict[flip_temp_right_predict >= 0.5] = 1.
        # flip_temp_left_predict[flip_temp_left_predict < 0.5] = .0
        # flip_temp_right_predict[flip_temp_right_predict < 0.5] = .0
        # print(seg_miou(right_val_predict, right_val_batch_Y))
        ...
        print(left_val_predict.shape)
        left_val_predict = np.transpose(left_val_predict, (1, 2, 0))
        right_val_predict = np.transpose(right_val_predict, (1, 2, 0))
        left_val_predict = np.array(left_val_predict[:, :, 0])
        right_val_predict = np.array(right_val_predict[:, :, 0])
        # left_val_predict = normalize(left_val_predict)
        # right_val_predict = normalize(right_val_predict)
        left_val_predict[left_val_predict >= 0.5] = 1
        # left_val_predict[left_val_predict < 0.5] = 0
        right_val_predict[right_val_predict >= 0.5] = 1
        # right_val_predict[right_val_predict < 0.5] = 0

        # left_val_predict = np.squeeze(left_val_predict, axis=0)
        # right_val_predict = np.squeeze(right_val_predict, axis=0)
        print(left_val_predict.shape)
        print(right_val_predict.shape)

        left_y_predict_resize = cv2.resize(left_val_predict, (left_x_original.shape[1], left_x_original.shape[0]))
        right_y_predict_resize = cv2.resize(right_val_predict, (left_x_original.shape[1], left_x_original.shape[0]))
        print(left_y_predict_resize.shape)
        print(right_y_predict_resize.shape)
        # if 'positive' in img_path:
        #     plt.imshow(y_predict_resize)
        #     plt.show()
        left_y_predict_resize = np.stack([left_y_predict_resize, left_y_predict_resize, left_y_predict_resize], axis=2)
        right_y_predict_resize = np.stack([right_y_predict_resize, right_y_predict_resize, right_y_predict_resize],
                                          axis=2)
        # end = time.time()
        ...
        # left_temp_y_predict[left_temp_y_predict >= 0.5] = 1
        # left_temp_y_predict[left_temp_y_predict < 0.5] = 0
        left_y_predict_resize *= 255
        left_temp_y_predict *= 255
        left_y_predict_resize = np.cast[np.uint8](left_y_predict_resize)
        left_temp_y_predict = np.cast[np.uint8](left_temp_y_predict)
        left_height, left_width = left_y_predict_resize.shape[:2]

        left_y_predict_resize = cv2.cvtColor(left_y_predict_resize, cv2.COLOR_BGR2GRAY)

        right_temp_y_predict = right_y_predict_resize.copy()

        # right_temp_y_predict[right_temp_y_predict >= 0.5] = 1
        # right_temp_y_predict[right_temp_y_predict < 0.5] = 0
        right_y_predict_resize *= 255
        right_temp_y_predict *= 255
        right_y_predict_resize = np.cast[np.uint8](right_y_predict_resize)
        right_temp_y_predict = np.cast[np.uint8](right_temp_y_predict)
        right_height, right_width = right_y_predict_resize.shape[:2]
        right_y_predict_resize = cv2.cvtColor(right_y_predict_resize, cv2.COLOR_BGR2GRAY)
        #if 'test' in img_path:
        if 'joint' in img_path:
            r_threshold = 1
            ...
            # left_r_thres = np.sum(left_r_thres)
            ...
            left_heatmap = cv2.applyColorMap(left_temp_y_predict, cv2.COLORMAP_JET)
            # heatmap = cv2.cvtColor(heatmap, cv2.COLOR_BGR2RGB)
            left_heatmap[(left_heatmap[:, :, 0] != 0) & (left_heatmap[:, :, 1] == 0) & (left_heatmap[:, :, 2] == 0)] = 0
            #
            right_heatmap = cv2.applyColorMap(right_temp_y_predict, cv2.COLORMAP_JET)
            # heatmap = cv2.cvtColor(heatmap, cv2.COLOR_BGR2RGB)
            right_heatmap[
            ...
            print(right_x_original.shape)
            print(right_heatmap.shape)
            # left_x_original = cv2.cvtColor(left_x_original, cv2.COLOR_GRAY2RGB)
            temp_left_superimposed_img = cv2.addWeighted(left_x_original, 0.8, left_heatmap, 0.2, 0)
            temp_right_superimposed_img = cv2.addWeighted(right_x_original, 0.8, right_heatmap, 0.2, 0)
            x_minus = int(x_minus)
            x_original_1 = x_original.copy()

            gap = int((x_original.shape[1] * 0.5) - (x_original.shape[1] * 0.4))
            left_superimposed_img = left_temp_y_predict[:, :]
            right_superimposed_img = right_temp_y_predict[:, :]
            left_superimposed_img = left_superimposed_img[:, 0:left_y_predict_resize.shape[1] - gap]
            right_superimposed_img = right_superimposed_img[:, 0 + gap:right_y_predict_resize.shape[1]]
            temp_left_superimposed_img = temp_left_superimposed_img[:, 0:left_y_predict_resize.shape[1] - gap]
            temp_right_superimposed_img = temp_right_superimposed_img[:, 0 + gap:right_y_predict_resize.shape[1]]
            # left_superimposed_img = left_y_predict_resize[:, 0:left_y_predict_resize.shape[1] - gap]
            # right_superimposed_img = right_y_predict_resize[:, 0 + gap:right_y_predict_resize.shape[1]]
            ...
            temp_overlayimg = cv2.hconcat([temp_left_superimposed_img, temp_right_superimposed_img])
            if x_original.shape[1] == x_original.shape[0] + (x_minus * 2):
                # x_original = cv2.copyMakeBorder(x_original, x_minus, x_minus, 0, 0, cv2.BORDER_CONSTANT)
                overlayimg = overlayimg[x_minus:x_original.shape[1] - x_minus, :]
                temp_overlayimg = temp_overlayimg[x_minus:x_original.shape[1] - x_minus, :]
            else:
                # x_original = cv2.copyMakeBorder(x_original, x_minus, x_minus + 1, 0, 0, cv2.BORDER_CONSTANT)
                overlayimg = overlayimg[x_minus + 1:x_original.shape[1] - x_minus + 1, :]
                temp_overlayimg = temp_overlayimg[x_minus + 1:x_original.shape[1] - x_minus + 1, :]

            # cv2.imwrite(save_y_pred + 'positive/right_' + img_name, right_superimposed_img)
            cv2.imwrite(save_y_pred + 'positive/' + img_name, overlayimg)
            cv2.imwrite(save_y_pred + 'negative/' + img_name, temp_overlayimg)
